...
func usage() string {
	return strings.Join([]string{
		...
		"",
		"Advanced/recovery:",
		"  `hyper complete` is the finish-gate command for agents and manual recovery; it is not the normal human entrypoint.",
		"",
		"Method:",
...
func commandUsage(command string) string {
	lines := map[string][]string{
		...
		"run": {
			...
			"  --until <stage>     Override plan.md Target Stage with tiny-mvp, usable-mvp, beta, service-quality, or sustained-service-quality.",
		},
		"status": {
			"Usage:",
			"  hyper status",
		...
		"complete": {
			...
			"  hyper complete",
			"",
			"Advanced/recovery command. Agents run this after evidence.md and next.md are written.",
			"Runs the finish gate, learns from the packet, then refreshes growth and readiness.",
		},
		"advance": {
			"Usage:",
			"  hyper advance",
