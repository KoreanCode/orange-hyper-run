...
Hyper Run은 AI 코딩 작업을 작은 MVP에서 더 큰 제품 단계까지 이어가기 위한 도구입니다.

프로젝트 루트에 `plan.md`를 적어두면, Hyper Run이 다음에 할 작은 작업 단위를 만들고, 진행 기록을 `.hyper/`에 저장하고, 완료 evidence를 바탕으로 다음 작업을 더 구체적으로 만듭니다.

Codex Desktop이나 CLI 기반 AI 코딩을 쓸 때, 프로젝트 안에서 반복할 기본 흐름은 이것입니다.
...
Hyper Run은 이 문맥을 프로젝트 안에 남깁니다. 복잡한 프로젝트 관리 도구가 아니라, 다음 작업 packet을 만들고 결과에서 배우는 작은 런타임입니다.
...
| Learn | 완료된 작업에서 재사용할 결정, 패턴, 제약, 실패를 뽑아내는 단계입니다. 단순 요약이 아닙니다. |
| Growth | 프로젝트가 반복되는 압력을 알아차리는 상태입니다. 예를 들어 매번 같은 검증이 필요하면 validator 후보를 만들 수 있습니다. |
| Readiness | Tiny MVP에서 Usable MVP, Beta, Service Quality로 넘어갈 준비가 됐는지 보는 간단한 단계 기준입니다. |
