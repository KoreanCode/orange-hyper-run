...
`plan.md`에 `Target Stage`가 있으면 plain `hyper run`이 그 목표까지 packet 단위로 계속 이어갑니다. 단, 무검토 자동 실행은 아닙니다. 각 packet은 evidence를 남기고, agent finish gate를 통과해야 하며, 다음 packet으로 갈지, 같은 packet을 고칠지, stage를 올릴지, 멈출지 결정합니다.
...
```text
plan.md -> hyper run -> goal.md/tasks.md -> evidence.md/next.md -> agent finish gate -> 다음 packet
```
...
flowchart TD
  ...
  E --> F["Agent finish gate<br/>packet 확인"]
  F --> G{"증거가 충분한가?"}
...
Agent finish gate는 배운 점을 저장하기 전에 packet을 먼저 확인합니다. validation, stage evidence, active check, `next.md`가 부족하면 현재 packet의 `review.md`에 보강할 내용을 남기고 agent가 같은 packet에 머무르게 합니다. 같은 finding은 `.hyper/next-packet.md`와 `hyper resume`에도 표시되어, 다음 Codex 단계가 새 작업을 시작하지 않고 현재 packet을 보강하게 합니다.
...
`hyper run`이 막히면 현재 packet을 먼저 finish gate까지 통과시켜야 합니다. Codex Desktop에서는 agent에게 현재 Hyper Run packet을 이어서 처리하라고 요청하면 됩니다. agent는 evidence/next를 보강하고 finish gate를 내부 실행해야 합니다. 수동 복구가 필요할 때는:
...
finish gate가 `review.md`를 작성했다면 새 `hyper run`을 시작하지 말고 같은 packet을 보강합니다.
...
이전 packet의 evidence가 아직 pending이면 새 `hyper run`은 막힙니다. 먼저 agent가 현재 packet을 finish gate까지 통과시켜야 합니다.

## Agent Finish Gate (`hyper complete`)

구현이 끝나면 agent가 `evidence.md`와 `next.md`를 업데이트한 뒤 내부적으로 실행합니다.
...
`hyper complete`는 통합되지 않은 agent나 디버그를 위한 advanced/manual recovery 명령으로도 남아 있습니다. 일반 사용자가 직접 수행하는 기본 entrypoint는 계속 `hyper run`입니다. 다음 `hyper run`은 이 정보를 사용합니다.
...
hyper complete              # agent/recovery finish gate; 일반 사용자 기본 entrypoint 아님
