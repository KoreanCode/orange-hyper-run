...
| Proof Contract | runtime packet이 이번 작업에서 무엇을 증명해야 하는지 정하는 짧은 경계입니다. functional proof, surface proof, operational proof로 나뉩니다. |
...
각 runtime packet은 세 가지를 짧게 증명하게 만듭니다.

- Functional proof: 가장 작은 유용한 동작이 실제로 되는지.
- Surface proof: 사용자-facing 화면이 바뀌었다면 사용자가 화면을 이해하고, 주요 행동을 하고, 결과나 복구 상태를 볼 수 있는지.
- Operational proof: build, test, smoke, setup, handoff 중 가장 안전한 검증 경로가 반복 가능한지, 아니면 blocker가 명확히 기록됐는지.

Surface proof는 디자인 하네스가 아닙니다. screenshot, browser, responsive, accessibility, Figma, design system 검증은 반복 evidence가 쌓였을 때만 더 강한 후보가 됩니다.
...
flowchart TD
  ...
  B --> C["hyper run [focus]<br/>runtime packet 생성"]
  C --> D["현재 GOAL 구현<br/>goal.md와 tasks.md 확인"]
  D --> E["evidence.md와 next.md 업데이트"]
  E --> F["hyper complete<br/>Finish Gate 실행"]
  F --> G{"Finish Gate 통과?"}
  G -- "아니오" --> H["review.md에 보강점 기록<br/>같은 GOAL에 머무름"]
  H --> D
  G -- "예" --> I["Learn<br/>memory, growth, readiness 갱신"]
  I --> J["Next Packet Planner<br/>.hyper/next-packet.md 작성"]
...
`hyper complete`는 학습하기 전에 finish gate를 먼저 실행합니다. validation, readiness evidence, active capability evidence, `next.md`가 부족하면 현재 packet의 `review.md`에 보강할 내용을 남기고 같은 packet에 머무르게 합니다.
...
auto mode는 evidence를 건너뛰거나 stage를 몰래 올리지 않습니다. 다음 packet 명령은 `.hyper/next-packet.md`에 계획하고, stage 변경은 여전히 사용자가 `hyper advance`로 받아들여야 합니다.
