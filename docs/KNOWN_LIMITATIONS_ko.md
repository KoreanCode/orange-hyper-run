...
- Auto mode는 packet 단위 continuation이지, 완전히 방치해도 되는 autonomous background runner는 아닙니다. `Target Stage`는 plain `hyper run`이 목표를 고르게 하지만 finish gate와 stage advance guard는 그대로 적용됩니다.
