...
- 패키지 경계는 아직 대부분 `cmd/hyper` 안에 있습니다. 현재 CLI 크기에서는 괜찮지만, 이후 분리가 필요합니다.
- 기존 `.hyper/` 프로젝트는 큰 growth/readiness 변경 뒤 `hyper migrate`가 필요할 수 있습니다.
- Windows release binary는 빌드되고 CI 테스트도 돌지만, 아직 PowerShell installer script는 없습니다.

## 보안과 공급망

- release build는 checksums를 제공합니다.
- installer는 GitHub release download에 대해 checksum을 검증합니다.
- release binary는 아직 서명되지 않았습니다. cosign 서명은 다음 단계입니다.
- `hyper update`도 `install.sh`와 같은 checksum 검증을 갖춰야 합니다.
