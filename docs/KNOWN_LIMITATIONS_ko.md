...
- Agent finish gate로 packet 종료 (`hyper complete`는 agent/manual recovery용)
- `plan.md` `Target Stage` 또는 `hyper run --auto --until <stage>`를 통한 guarded auto continuation
...
- Capability candidate는 명시적인 threshold 기반 activation policy를 사용합니다. 이 정책은 의도적으로 보수적이며, 팀별 custom threshold는 아직 지원하지 않습니다.
- Auto mode는 packet 단위 continuation이지, 완전히 방치해도 되는 autonomous background runner는 아닙니다. `Target Stage`는 plain `hyper run`이 목표를 고르게 하고, 해당 target stage의 readiness proof가 완료된 뒤에만 멈춥니다. finish gate는 그대로 적용되며, stage advancement는 Stage Advancement Review가 ready proof와 blocking gap 없음 상태를 보여줄 때만 이어집니다. blocked/waiting packet은 명시적인 focus가 있는 follow-up이 필요하고, `.hyper/next-packet.md`에는 진전 없는 반복 loop를 멈추기 위한 Progress Guard도 포함됩니다.
- Reference benchmark evidence는 구조화되어 있고 검증되지만, 비교의 품질은 여전히 agent나 개발자가 남긴 evidence 품질에 의존합니다.
- 패키지 경계는 아직 대부분 `internal/app` 안에 있습니다. stage vocabulary는 `internal/stage`로 분리했고, 더 큰 policy/storage/runtime 분리는 실제 pressure가 생겼을 때 진행해야 합니다.
...
- 좋은 evidence는 여전히 agent가 `evidence.md`와 `next.md`를 성실하게 업데이트해야 합니다. 수동 개발자는 recovery/debugging 때 같은 작업을 할 수 있습니다.
