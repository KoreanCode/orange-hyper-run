...
9. `.hyper/next-packet.md`에 다음 명령과 progress guard를 명시해 auto continuation도 packet 단위로 검토 가능하고, 진전 없는 반복 loop에서는 멈출 수 있게 유지합니다.
...
CLI에서 `hyper run`을 한 번 실행하면 최대 하나의 runtime packet을 만듭니다. `plan.md`에 `Target Stage`가 있으면 plain `hyper run`은 guarded auto mode로 들어가고, 각 packet이 완료될 때마다 다음 continuation 명령을 기록합니다. Target은 "이 stage에 진입"이 아니라 "이 stage의 readiness proof를 완료"한다는 뜻입니다. 예를 들어 `Target Stage: Service Quality`는 Service Quality gate proof가 완료될 때까지 Service Quality packet 안에서도 계속 진행합니다. 목표가 `plan.md`에서 왔을 때 continuation 명령은 계속 plain `hyper run`으로 유지하고, `--auto --until`은 명령어에서 직접 override할 때만 사용합니다. 그 packet은 실행 agent가 다음을 완료했을 때 끝난 것으로 봅니다.
...
`hyper run`을 unchecked background loop로 보면 안 됩니다. 길게 이어지는 부분은 packet 단위 continuation입니다. 하나의 packet을 만들고, 실행하고, evidence를 확인하고, 학습한 뒤, guard가 허용할 때만 `.hyper/next-packet.md`의 다음 명령을 따릅니다. 이 파일에는 다음 명령, Codex Desktop continuation 안내, progress guard가 들어갑니다. progress guard는 같은 명령이나 같은 review finding이 새 evidence, stage 이동, next-packet plan 변경 없이 반복되면 agent가 멈춰서 loop risk를 보고하게 합니다. 이전 active packet의 evidence가 아직 pending이면 새 `hyper run`은 차단됩니다. packet이 `blocked` 또는 `waiting_user`로 닫히면 plan target을 쓰는 plain `hyper run`도 새 packet을 만들지 않고 멈춥니다. blocker나 사용자 결정이 해결된 뒤 명시적인 focus를 넣어 deliberate follow-up을 시작해야 합니다.
...
Beta와 Service Quality stage에서는 repeatable smoke, security, deployment, operations check를 위한 quiet validator candidate를 만들 수 있습니다. 이들은 repeated evidence로 active validator가 되기 전까지 candidate로 남습니다. 이 lifecycle은 `.hyper/growth/state.json`과 각 capability file에 명시됩니다. repeated evidence는 candidate를 만들고, promotion 수준의 evidence는 검토 가능한 상태로 만들며, activation 수준의 evidence 또는 maintainer가 설치한 active file만 future finish gate의 required behavior가 됩니다.
...
Hyper Run은 category, 3-5개의 named reference, baseline expectation, 현재 below/meets/above-baseline 비교, critical below-baseline gap 없음, 하나 이상의 above-baseline strength, 그리고 Service Quality 진행을 명시적으로 허용하는 decision이 모두 있을 때만 이 evidence를 covered로 봅니다. `blocked`, `not ready`, `allowed only after more work` 같은 decision은 packet을 계속 열어둡니다. 현재 gate에서 필요하면 `hyper status`에 benchmark line이 직접 표시됩니다.
...
- Reference Benchmark Evidence가 Service Quality를 blocked, not ready, 또는 추가 작업 이후에만 가능하다고 판단합니다.
...
- Threshold 기반 activation policy 뒤에서 조용히 생성되는 validator, skill, harness candidate
