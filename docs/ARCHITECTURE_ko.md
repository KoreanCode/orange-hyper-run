...
| `internal/stage` | plan parsing, auto target, readiness, status output이 공유하는 canonical stage vocabulary, target-stage alias, stage ordering을 담당합니다. |

## 방향

이 변경은 첫 번째 package boundary입니다. 동작을 바꾸지 않고 `cmd/hyper`에서 제품 로직을 제거합니다.

대부분의 domain behavior는 아직 `internal/app` 안에 있지만, target-stage behavior가 plan parsing, readiness, auto continuation, status check 전반에서 반복 사용되므로 stage vocabulary는 분리했습니다. 이후 분리도 이처럼 작고 pressure가 증명된 단위로 진행합니다.
